/* =========================================================
   Plutoo – app.js
   - Autocomplete razze da breeds.json
   - Swipe fluido; niente “messaggi 10 like”
   - Match animation ~2.4s (chiudibile con tap)
...
(async function loadBreeds(){
  try{
    ...
    if(r.ok){ ALL_BREEDS = await r.json(); }
  }catch(_){ ALL_BREEDS = []; }
  const dl = $('#breedList');
  if(dl && Array.isArray(ALL_BREEDS)){
    dl.innerHTML = ALL_BREEDS.map(b=>`<option value="${b}">`).join('');
  }
})();
...
function renderLove(){
  ...
  if(cardEl){ cardEl.classList.remove('pulse'); void cardEl.offsetWidth; cardEl.classList.add('pulse'); attachSwipe(cardEl, d, 'love'); }
...
function swipeLove(type,d){ if(type==='yes'){ addMatch(d); showMatchAnim(d); } swipeLoveIdx++; renderLove(); }
...
function renderSocial(){
  ...
  if(cardEl){ cardEl.classList.remove('pulse'); void cardEl.offsetWidth; cardEl.classList.add('pulse'); attachSwipe(cardEl, d, 'social'); }
...
/* ---- Swipe helpers ---- */
function attachSwipe(cardEl, dogObj, mode){
  ...
  const onStart = (e)=>{ const t=e.touches?e.touches[0]:e; startX=currentX=t.clientX; startY=currentY=t.clientY; dragging=true; hasMoved=false; cardEl.style.transition='none'; };
  const onMove  = (e)=>{ if(!dragging) return; const t=e.touches?e.touches[0]:e; currentX=t.clientX; currentY=t.clientY;
  ...
  const onEnd   = ()=>{ if(!dragging) return; dragging=false; const dx=currentX-startX;
    cardEl.style.transition='transform .18s ease-out, opacity .18s ease-out';
    if(dx>80){ cardEl.style.transform='translateX(40%) rotate(6deg)'; cardEl.style.opacity='0';
      setTimeout(()=> (mode==='love'?swipeLove('yes',dogObj):swipeSoc('yes',dogObj)),180);}
    else if(dx<-80){ cardEl.style.transform='translateX(-40%) rotate(-6deg)'; cardEl.style.opacity='0';
      setTimeout(()=> (mode==='love'?swipeLove('no',dogObj):swipeSoc('no',dogObj)),180);}
  ...
  cardEl.addEventListener('mousedown',(e)=>{ onStart(e); const mm=(ev)=>onMove(ev); const mu=()=>{ onEnd(); document.removeEventListener('mousemove',mm); document.removeEventListener('mouseup',mu); }; document.addEventListener('mousemove',mm); document.addEventListener('mouseup',mu,{once:true}); });
}
...
function openProfilePage(d, distance){
  ...
  function render(){
    ...
    $('#ppSendVerify')?.addEventListener('click', ()=>{
      // simulazione annuncio prima della “verifica”
      ...
      }, 'Grazie! La verifica partirà subito dopo il video.');
      openDialogSafe(dlg);
    });

    // sblocco selfie con video se non c'è match
    $('#unlockSelfie')?.addEventListener('click', ()=>{
      const dlg = buildInterstitial(()=>{
        setSelfieGate(d.id, Date.now()); render();
      }, 'Sblocca i selfie per 24 ore');
      openDialogSafe(dlg);
    });
  }
...
function showMatchAnim(d){
  ...
  wrap.innerHTML = `
    <div style="text-align:center">
      <div style="font-size:72px;line-height:1;animation:pop .4s ease">❤️</div>
      <div style="margin-top:10px;display:flex;gap:12px;align-items:center;justify-content:center">
        <img src="${d.image}" alt="" style="width:72px;height:72px;border-radius:50%;object-fit:cover;animation:bump .4s ease">
        <div style="font-size:32px;animation:kiss .9s ease infinite">💋</div>
        <div class="paw" style="width:36px;height:36px;background-size:contain"></div>
      </div>
      <div style="margin-top:12px">È un match con <strong>${d.name}</strong>!</div>
      <div class="muted small" style="margin-top:6px">Tocca per chiudere</div>
    </div>
    <style>
  ...
  wrap.addEventListener('click', remove, {once:true});
...
document.addEventListener('DOMContentLoaded', ()=>{
  $('#ctaEnter')?.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); goHome(); }, {passive:false});
