...
// Riferimenti alle liste
const inboxList  = document.getElementById("tabInbox");
const sentList   = document.getElementById("tabSent");
const matchList  = document.getElementById("tabMatches");
...
chats.forEach((data) => {
  ...
  const dog =
    DOGS.find((d) => d.id === data.dogId) || null;
  const avatar = dog?.img || "plutoo-icon-192.png";
  const name   = dog?.name || "Dog";

  const isSent     = data.lastSenderUid === selfUid;
  const isReceived = data.lastSenderUid && data.lastSenderUid !== selfUid;
  const hasMatch   = !!data.match;

  // RICEVUTI
  if (isReceived && inboxList) {
    ...
    row.innerHTML = `
      ...
      <div class="msg-main">
        ...
        <div class="msg-preview">${text}</div>
