...
// ✅ FIX DEFINITIVO: una sola sorgente di verità per la lingua e per tutte le funzioni che usano window.state
 ...
 let nextMatchColor = ["🩵","🩷","💛","🧡","💚","💙","💜","💗","🫶","❤️"][state.matchCount % 10];
  ...
  try {
    ...
    // ✅ demo sempre presenti come base
    const demoStories = Array.isArray(this.generateMockStories)
      ? this.generateMockStories()
      : this.generateMockStories();

    const demoOnly = Array.isArray(demoStories)
      ? demoStories.filter(story => story && story.isDemo === true)
      : [];
    ...
    this.stories = [...demoOnly, ...firebaseStories];
    this.saveStories();
    ...
    if (saved) {
      ...
      const demoStories = this.generateMockStories();
      this.stories = Array.isArray(demoStories)
        ? demoStories.filter(story => story && story.isDemo === true)
        : [];
      this.saveStories();
    }
